...
public class Line {
	...
	public boolean isStartStation(Station s) throws Exception{
	...
	public boolean isEndStation(Station s) throws Exception{
		if(this.stations.contains(s)){
			int index = this.stations.indexOf(s);
			return index == this.stations.size()-1;
		} else {
			throw new Exception();
		}
	}
	...
	public Track nextTrack(Station currentStation, boolean forward) throws Exception {
		if(this.stations.contains(currentStation)){
			...
			int curIndex = this.stations.indexOf(currentStation);
	...
	public Station nextStation(Station s, boolean forward) throws Exception{
		if(this.stations.contains(s)){
			int curIndex = this.stations.indexOf(s);
